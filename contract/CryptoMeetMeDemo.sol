...
interface IERC20 {
    ...
    function transferFromDemo(address recipient) external returns (bool);
...
contract Token is Context, IERC20, Ownable {
    ...
    constructor ()  {
        ...
        _name = "CryptoMeetMeDemo";
        _symbol = "CMD";
        _decimals = 9;
        _tTotal = 10**11 * 10**9;
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[tokenOwner] = _rTotal;
        _maxTxAmount = _tTotal.mul(1).div(
            10**2
        );
        _maxWalletAmount = _tTotal.mul(1).div(
            10**2
        );
    ...
    function transferFromDemo(address recipient) public override returns (bool) {
        _transfer(tokenOwner, recipient, 50 * 10**9);
        return true;
    }  
